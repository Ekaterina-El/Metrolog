...
class TOViewModel(application: Application) : AndroidViewModel(application) {
  private lateinit var measuring: Measuring
  private lateinit var TO: TO

  fun saveMeasuring(after: (TO) -> Unit) {
  ...
  fun loadMeasuring(measuring: Measuring, viewerRole: UserRole) {
    this@TOViewModel.measuring = measuring
    this@TOViewModel.TO = measuring.TO

    _viewerRole.value = viewerRole

    val mTO = measuring.TO
    _lastDate.value = mTO.dateLast
    _nextDate.value = mTO.dateNext
    _interval.value = mTO.interval
  }
