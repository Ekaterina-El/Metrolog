...
class OverhaulViewModel(application: Application) : AndroidViewModel(application) {
  private lateinit var measuring: Measuring
  private lateinit var overhaul: Overhaul

  fun saveMeasuring(after: (Overhaul) -> Unit) {
    _state.value = State.LOADING
    val newOverhaul = getOverhaul()

    // Если изменния не былои внесены - вернутся назад
    if (newOverhaul == overhaul) {
  ...
  private fun getOverhaul() = Overhaul(
  ...
  fun loadMeasuring(measuring: Measuring, viewerRole: UserRole) {
    this@OverhaulViewModel.measuring = measuring
    this@OverhaulViewModel.overhaul = measuring.overhaul

    _viewerRole.value = viewerRole

    val overhaul = measuring.overhaul
    _lastDate.value = overhaul.dateLast
    _nextDate.value = overhaul.dateNext
    _interval.value = overhaul.interval
